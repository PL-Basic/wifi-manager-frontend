...
<template><section class="workspace-view security-page"><header class="dashboard-header"><div><p class="page-kicker">安全工作区</p><h2>审计</h2></div><button class="secondary-button" :disabled="loading" @click="load()"><RefreshCw :size="16" />刷新</button></header><p v-if="error" class="alert error security-inline-message">{{ error }}</p><form class="glass-toolbar security-toolbar" @submit.prevent="search"><label><span>动作</span><input v-model="filters.action" placeholder="alert.handle" /></label><label><span>操作人</span><input v-model="filters.operatorName" /></label><label><span>目标</span><input v-model="filters.target" /></label><label><span>开始时间</span><input v-model="filters.startTime" type="datetime-local" /></label><label><span>结束时间</span><input v-model="filters.endTime" type="datetime-local" /></label><div class="security-actions"><button type="submit" :disabled="loading"><Search :size="16" />查询</button><button class="secondary-button" type="button" :disabled="loading" @click="reset">重置</button></div></form><StateBlock v-if="loading && !loaded" type="loading" title="正在加载审计日志" /><StateBlock v-else-if="loaded && !error && !rows.length" title="暂无审计日志" /><section v-if="loaded && rows.length" class="glass-panel security-table-wrap"><table class="security-table"><thead><tr><th>ID</th><th>动作</th><th>操作人</th><th>目标</th><th>来源 IP</th><th>创建时间</th><th>操作</th></tr></thead><tbody><tr v-for="row in rows" :key="row.id"><td>{{ row.id }}</td><td>{{ row.action || '-' }}</td><td>{{ row.operatorName || '-' }}</td><td class="wrap-cell">{{ row.target || '-' }}</td><td>{{ row.ip || '-' }}</td><td>{{ formatTime(row.createTime) }}</td><td><button class="icon-button" type="button" title="查看审计详情" @click="open(row.id)"><Eye :size="16" /></button></td></tr></tbody></table></section><AppPagination v-if="loaded" :current="pager.current" :size="pager.size" :total="pager.total" :busy="loading" @change="load"/><AuditDetailDrawer :open="drawerOpen" :audit-id="selectedId" @close="close" /></section></template>
