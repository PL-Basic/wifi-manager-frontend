...
function submit() {
  ...
  if (!editing.value && !/^[a-z][a-z0-9-]{2,63}$/.test(tenantCode)) {
    localError.value = '组织标识需以小写字母开头，只能使用小写字母、数字和短横线，长度为 3 至 64 位'
    return
  }
  if (!name || name.length > 128) {
    localError.value = '组织名称不能为空，且不能超过 128 个字符'
    return
  }
  if (!timezone || timezone.length > 64) {
    localError.value = '请输入有效时区，例如 Asia/Shanghai'
    return
  }
      ...
      <section class="platform-modal" role="dialog" aria-modal="true" aria-labelledby="tenant-form-title">
        <header>
          <div class="platform-modal-title">
            ...
            <div><p>组织资料</p><h3 id="tenant-form-title">{{ editing ? '编辑组织' : '创建组织' }}</h3></div>
          </div>
        ...
        <form class="platform-form" @submit.prevent="submit">
          <label v-if="!editing"><span>组织标识</span><input ref="codeInput" v-model="form.tenantCode" autocomplete="off" maxlength="64" placeholder="例如 team-alpha" :disabled="submitting" /></label>
          <label><span>组织名称</span><input ref="nameInput" v-model="form.name" maxlength="128" placeholder="请输入组织名称" :disabled="submitting" /></label>
          <label><span>所在时区</span><input v-model="form.timezone" maxlength="64" placeholder="例如 Asia/Shanghai" :disabled="submitting" /></label>
          <div class="platform-modal-actions">
            <button class="secondary-button" type="button" :disabled="submitting" @click="close">取消</button>
            <button type="submit" :disabled="submitting">{{ submitting ? '正在保存...' : editing ? '保存修改' : '创建组织' }}</button>
          </div>
        </form>
      </section>
    </div>
